...
## Observe your API traffic with ngrok and Datadog
...
You can monitor API services in two ways in ngrok: with the [Traffic Inspector](/obs/traffic-inspection/) and with [Events](/obs/events).
...
- Browse to the ngrok [Traffic Inspector](https://dashboard.ngrok.com/traffic-inspector) on your ngrok account dashboard.
  ...
  ![Traffic Inspector](/guides/api-gateway/img/monitor-ngrok/trafficInspector.png)
...
- Click a request in the Traffic Inspector list, then click **Enable full capture** in the sidebar.
  ...
  ![Enable full capture](/guides/api-gateway/img/monitor-ngrok/enableFullCapture.png)

- This button takes you to your account settings, where you can enable full capture under **Observability**.

  ![Enable full capture settings](/guides/api-gateway/img/monitor-ngrok/enableFullCaptureAccount.png)
...
- Click the **Replay** button and notice that the request is replayed from the ngrok host.
  ...
  ![Replay request](/guides/api-gateway/img/monitor-ngrok/replay.png)

- You can also replay a request with changes to alter any of the headers or POST data.

  ![Replay with changes](/guides/api-gateway/img/monitor-ngrok/replayChanges2.png)
...
- In Traffic Inspector, note that ngrok returned the error code `429` (rate limit) and that the duration of the request was instant, meaning it created no load on your API.
  ...
  ![Rate limit](/guides/api-gateway/img/monitor-ngrok/rateLimit.png)
...
- In the Traffic Inspector, click on one of the `429` events, then click **Replay**.
  ...
  ![Replay after rate limit adjustment](/guides/api-gateway/img/monitor-ngrok/replayLimit.png)
...
- In the ngrok dashboard, browse to the [**Events Stream**](https://dashboard.ngrok.com/event-subscriptions) and add a new subscription.
  ...
  ![Add event subscription](/guides/api-gateway/img/monitor-ngrok/makeSubscription.png)
...
- In the **New Event Subscription** sidebar, enter `traffic` as the **Description** and then **Add** a new source (event type).
  ...
  ![Add source](/guides/api-gateway/img/monitor-ngrok/addEventType.png)
...
- Open the Datadog site and browse to the **Log Explorer** page from the navigation panel.
  ...
  ![Datadog logs](/guides/api-gateway/img/monitor-ngrok/datadogLog.png)
...
- In the Datadog site, browse to the **Dashboards > Dashboard List** page from the navigation panel.
  ...
  ![Datadog dashboard list](/guides/api-gateway/img/monitor-ngrok/datadogDashboardList.png)

You need to clone the default dashboard to add a new widget to the dashboard.

- Click **Clone** at the top right of the page.

  ![Datadog default dashboard](/guides/api-gateway/img/monitor-ngrok/datadogDefaultDashboard.png)

- In the cloned dashboard, click **Add widgets**.

  ![Datadog add widget](/guides/api-gateway/img/monitor-ngrok/datadogAddWidget.png)
...
- **Step 4:** Name the widget `Errors in the last hour`.
  ...
  ![Datadog edit widget](/guides/api-gateway/img/monitor-ngrok/datadogWidgetDetails.png)
...
![Datadog custom dashboard](/guides/api-gateway/img/monitor-ngrok/datadogCustomDashboard.png)
...
- In Datadog, browse to **Integrations > Add integration > Webhooks**.
  ...
  ![Datadog add integration](/guides/api-gateway/img/monitor-ngrok/datadogAddIntegration.png)
...
- Under **Webhooks**, at the bottom of the configuration page, click **New**.
  ...
  ![Datadog add webhook](/guides/api-gateway/img/monitor-ngrok/datadogAddWebhook.png)
...
- In the monitor configuration, choose **Query** for your monitor scope, set the search query to `@http.status_code:500`, the time to the **last 1 hour**, and the notification message:
  ...
  ![Datadog add webhook](/guides/api-gateway/img/monitor-ngrok/datadogConfigureMonitor.png)
...
- Notice that the alert appears on the ntfy page.
  ...
  ![Datadog notify](/guides/api-gateway/img/monitor-ngrok/datadogNotify.png)
...
As mentioned earlier, ngrok emits [audit events](/obs/events/reference/#audit-events) for any security changes.
