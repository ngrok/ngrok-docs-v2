...
import Back from "/snippets/universal-gateway/_back-to-examples.mdx";
...
**What's happening here?** This policy filters for _only_ traffic that contains the specific `/webhook/$WORKFLOW_UUID` path, then verfies that it both originates from GitHub (or another [supported provider](/traffic-policy/actions/verify-webhook/#supported-providers)) and contains the appropriate secret.
