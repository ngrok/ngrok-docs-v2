...
<ContentSwitcher languages={["bash"]}>
  ...
  the internet and secure it so that only you can access it. It assumes you
  already have your own application running locally.
</ContentSwitcher>

<Definition>IP CIDR</Definition>

<Callout type="tip">
  An IP CIDR (Classless Inter-Domain Routing) is a notation used to specify IP
  addresses and their associated network masks. It is commonly used in routing
  and network configuration to define a range of IP addresses.
</Callout>
...
```bash title="ngrok with static domain" tabName="Hello"
ngrok http 8080 --url https://jumpy-red-mollusk.ngrok-free.app
```

```sh title="ngrok with static domain" tabName="SSH"
example
```

```go title="ngrok with static domain"
example 2
```
