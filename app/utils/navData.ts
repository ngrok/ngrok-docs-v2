/** Future cleanup:
 * Update the link prop to just be a string with the URL
 * Remove the type prop
 * Remove className prop
 * Remove collapsible prop
 */
...
export type NavBucket = {
  ...
  items: Array<string | SidebarGroup>;
  collapsible?: boolean;
  className?: string;
};
...
export type SidebarGroup = {
  ...
  link?: { type: "doc"; id: string } | { type: "link"; href: string };
  items?: Array<string | SidebarGroup>;
  id?: string; // For docs
};

export const navData: NavBucket[] = [
  {
    label: "Overview",
    type: "bucket",
