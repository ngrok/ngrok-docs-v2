...
export type SidebarItemData = {
  type?: "doc" | "category" | "link" | "autogenerated" | "bucket";
  title: string;
  path?: string;
  href?: string;
  ...
  try {
    ...
    for await (const dirent of dir) {
      ...
      itemsList.push({
        ...
        explicitSidebarPosition: fileOfRemark.data?.frontmatter?.sidebar_position,
        children: [],
      });
    }
  } catch (error) {
...
const getItemFromString = async (
  ...
  return {
    ...
    explicitSidebarPosition: data.frontmatter?.sidebar_position,
    children: [],
  };
};
...
function findInNestedArray(children: any, path: string) {
  ...
  for (const item of children) {
    ...
    if(item["path"] && doNormalizedPathsMatch(path, item["path"])) {
        return item;      
    }
